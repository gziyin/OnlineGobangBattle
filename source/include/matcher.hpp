...
inline void Matcher::init(OnlineManager* online_mgr) {
    _online_mgr = online_mgr;
}

inline void Matcher::set_match_callback(MatchCallback cb) {
    _match_callback = cb;
}

inline bool Matcher::enqueue(int64_t user_id, int score) {
...
inline bool Matcher::cancel(int64_t user_id) {
    bool erased = false;
    {
        std::lock_guard<std::mutex> lock(_user_map_mtx);
...
inline void Matcher::on_disconnect(int64_t user_id) {
    (void)cancel(user_id);
}

inline void Matcher::start() {
    bool expected = false;
...
inline MatchTier Matcher::score_to_tier(int score) {
    if (score < 1300) {
...
inline int Matcher::tier_index(MatchTier tier) {
    return static_cast<int>(tier);
}

inline int64_t Matcher::now_ms() {
    return std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now().time_since_epoch()
    ).count();
}

inline void Matcher::match_loop() {
    while (_running.load()) {
...
inline void Matcher::drain_incoming_requests() {
    for (int tier = 0; tier < 3; ++tier) {
        drain_tier(tier);
    }
}

inline void Matcher::drain_tier(int tier) {
    MatchRequest req;
...
inline void Matcher::try_match_same_tier(int tier) {
...
inline void Matcher::try_match_cross_tier(int tier) {
...
inline bool Matcher::discard_invalid_front(int tier) {
    std::deque<MatchRequest>& queue = _pending_queues[tier];
    if (queue.empty()) {
        return false;
    }
...
inline bool Matcher::is_request_valid(const MatchRequest& req) const {
    if (_online_mgr == nullptr) {
        return false;
    }
    if (req.cancelled) {
...
inline bool Matcher::should_allow_cross_tier(const MatchRequest& left,
...
inline void Matcher::finalize_match(const MatchRequest& p1, const MatchRequest& p2) {
    {
        std::lock_guard<std::mutex> lock(_user_map_mtx);
...
inline int64_t Matcher::generate_room_id() {

...
| 阶段 | 名称 | 周期 | 主要目标 | 完成状态 |
|------|------|------|----------|----------|
| M1 | 环境搭建与基础框架 | 第1-2周 | 完成开发环境，搭建 HTTP/WebSocket 服务器框架 | ⏳ |
| M2 | 数据库与用户模块 | 第3-4周 | 完成数据库设计、用户注册/登录/安全功能 | ⏳ |
| M3 | 在线管理与匹配模块 | 第5-6周 | 完成在线用户管理、匹配对战功能 | ⏳ |
| M4 | 游戏房间与对战逻辑 | 第7-8周 | 完成房间管理、五子棋对战核心逻辑、断线重连 | ⏳ |
| M5 | 聊天功能与优化 | 第9周 | 完成实时聊天、敏感词过滤、性能优化 | ⏳ |
| M6 | 测试与部署 | 第10周 | 完成测试、部署、文档编写 | ⏳ |
...
| 天数 | 任务 | 交付物 | 完成状态 |
|------|------|--------|----------|
| Day 1-3 | WebSocket++ 服务器框架<br>- HTTP 请求路由<br>- WebSocket 连接管理<br>- 回调函数注册机制 | 基础服务器框架 | ⏳ |
| Day 4-5 | 日志模块<br>- 分级日志(INFO/WARN/ERROR)<br>- 异步写入<br>- 日志轮转 | logger.hpp/cpp | ⏳ |
| Day 6-7 | 工具模块<br>- JSON 序列化/反序列化<br>- 时间工具<br>- 字符串处理 -文件读取功能 -数据库的连接和初始化 | util.hpp/cpp | ⏳ |
...
| 天数 | 任务 | 交付物 | 完成状态 |
|------|------|--------|----------|
| Day 1-2 | 数据库设计实现<br>- 用户表、房间表、比赛记录表<br>- 创建表脚本<br>- **简易连接池（`std::queue<MYSQL*>`）** | db.hpp/cpp | ⏳ |
| Day 3-5 | 安全模块（⚠️ 提前验证依赖可用性）<br>- bcrypt 密码加密（备选：OpenSSL PBKDF2）<br>- JWT Token 生成/验证（jwt-cpp header-only）<br>- 限流器实现<br>> 注：三个组件各有踩坑风险，单独留 3 天 | security.hpp/cpp | ⏳ |
| Day 6 | 用户数据访问层<br>- UserTable 类<br>- 注册/登录/查询接口<br>- 分数更新逻辑 | user_table 实现 | ⏳ |
...
| 天数 | 任务 | 交付物 | 完成状态 |
|------|------|--------|----------|
| Day 1-3 | HTTP API 实现<br>- /api/v1/auth/register<br>- /api/v1/auth/login<br>- /api/v1/auth/logout<br>- /api/v1/user/info | 认证 API 完成 | ⏳ |
| Day 4-5 | 会话管理<br>- Session 类设计<br>- Token 生命周期管理<br>- 登录状态保持 | session.hpp/cpp | ⏳ |
